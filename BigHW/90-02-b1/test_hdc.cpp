	...
	if (test_choise[T_MATHCURVE] == '1') {
		...
		*/
		...
		struct {
			...
			const char* name;
			void (*fun)(void);
		} hdc_draw_Mathematical_curve_lxy[] = {
			{2453359, "李欣洋", hdc_draw_Mathematical_curve_2453359},
			{0,NULL,NULL}
		};
		for (i = 0; hdc_draw_Mathematical_curve_lxy[i].sno > 0; i++) {
			hdc_draw_Mathematical_curve_lxy[i].fun();
			to_be_continued("数学曲线-", hdc_draw_Mathematical_curve_lxy[i].name, "-完成", STL_X, STL_Y);
		}
	}
	...
	if (test_choise[T_CUSTOM] == '1') {
		...
		*/
		struct {
			...
			int x;
			int y;
		} hdc_draw_cartoon_lxy[] = {
			{2453359, "李欣洋", hdc_draw_cartoon_2453359, 400, 400},
			{0,NULL,NULL}
		};
		for (i = 0; hdc_draw_cartoon_lxy[i].sno > 0; i++) {
			hdc_draw_cartoon_lxy[i].fun(hdc_draw_cartoon_lxy[i].x, hdc_draw_cartoon_lxy[i].y); //为了统一，此处的参照点坐标定死为（400,400）
			to_be_continued("卡通图像-", hdc_draw_cartoon_lxy[i].name, "-完成", STL_X, STL_Y);
		}
	}
