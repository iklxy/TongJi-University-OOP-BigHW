...
private:
	...
	/* ---------------------------------------------------------------
	   ...
	   ---------------------------------------------------------------- */
	char pad[68];
	unsigned int ipv4ToUint(int a, int b, int c, int d);
	bool is_valid_ipv4(const string& ip);
	const char* to_string() const;
	const string get_range()const;//此函数用于输出其他类型的错误输出 以及最后打印时最后一列的范围
	const string get_range_double()const;//此函数用于错误输出double的范围
public:
	args_analyse_tools();
